...
manifest 以下键为**必填**（校验器强制；缺失即校验失败）：`command`、`started_utc`、`schema_version`
（须与顶层一致且为 1.0.0）、`config`（含 model/temperature）、`code_sha`、`code_dirty`、`model`、
`embed_model`、`llm_backend`、`embedder`、`temperature`、`seed`、`dataset`。

| 字段 | 类型 | 说明 |
|---|---|---|
| command | string | CLI 子命令与关键参数（如 `hotpot --n 10 --seed 7`） |
| started_utc | string | ISO 8601 UTC（秒精度），**run 开始时刻**（命令入口采集，非落档时刻） |
| schema_version | string | 与顶层一致 |
| code_sha | string \| null | `git rev-parse HEAD`；非 git 环境为 null（不猜测），配合 git_status 区分 |
| git_status | string | `ok` \| `unavailable`（审计可区分失败原因） |
...
| dataset | object \| null | `{path, sha256, n_items}`；**run 开始前采集**；合成任务为 null |
...
/ `*_metrics`，可嵌套一层）**必须**包含以下 V3-02 计量契约（缺失/类型错/非负性破坏即校验失败）：

| 字段 | 口径 | 说明 |
|---|---|---|
| llm_input_tokens / llm_output_tokens | chat | 真实 LLM API usage 分列（缺失 fail，禁止静默计 0） |
| transport_bytes | 传输 | Σ `len(to_wire())`：每条消息真实序列化帧长（帧内只含 CAS 句柄） |
| wire_bytes = logical_bytes | 逻辑 | Σ header+text+nontext：header+payload 恒等式右侧 |
| header_bytes / text_bytes / nontext_bytes | 逻辑 | 可复算基础量（校验器复算恒等式） |
| embed_requests / embed_cache_hits / embed_input_tokens | embed | 嵌入 API 请求（cold）/ 缓存命中（warm）/ usage token 分列 |
| cas_writes / cas_write_bytes | 状态面 | CAS put 次数/字节（共享状态建立成本） |

**恒等式（校验器复算 + 对账测试锁定）**：`transport_bytes == Σ逐消息 len(to_wire())`；
`logical_bytes == header_bytes + text_bytes + nontext_bytes`；`wire_bytes == logical_bytes`；
`aggregate == Σ trajectory`（全字段守恒，quality 为均值量除外）。
**注意**：transport 与 logical 之间**无大小关系约定**——to_wire() 帧只含句柄引用而 logical 计入
非文本载荷全量，真实路径两者任一方向都可能（句柄引用 vs 载荷计入是不同口径，非同一量的两种算法）。

**失败 run 豁免**：`result.status == "error"` 且 `result.error` 为非空字符串时豁免 metrics 契约
（失败 run 也是证据，须可落档可索引）；仅存在错误键而无显式 status 不构成豁免。
...
CoQA 为逐 turn 展开层（`qid = "<conv_id>:<turn_idx>"`，同样落顶层 `per_item`；
`result.per_conversation` 保留会话级结构）。逐题层使 F1/EM 可离线复算。
