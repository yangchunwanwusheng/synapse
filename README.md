...
- **系统技术落地：** Executor 经 **CodeAct** 执行模型生成代码，执行器双档可选——进程内受限解释器（默认）或**进程级隔离执行器**（`codeact_executor: subprocess`：超时可强杀直接 worker + POSIX 资源限制，实测边界见 `docs/codeact-boundary-report-*.md`；属进程边界故障/资源遏制，非安全沙箱——同用户文件系统/网络未隔离）；状态载荷通过进程内 CAS 内容句柄去重与按需恢复，记忆检索采用纯 Python 的关键词、标签与向量余弦融合；整体目标环境为 **openEuler 24.03-LTS-SP3**。当前 `master` 尚未提供跨进程共享内存、Socket、eBPF 或 faiss 后端，相关能力只在决赛工程路线图中推进，不作为已实现能力计分。
