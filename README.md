...
实验设计包含**不少于 2 组关联性连续任务**：HotpotQA（bridge 型并行多跳）与 MuSiQue（链式 two-hop 多跳）构成第一组多跳结构对照，CoQA（3 段对话合计 38 个问答轮，分别 12/11/15；来源 runs/coqa_20260724_211318，runs/ 暂未随仓分发、完整证据链随 P4.3 入库）与合成 G1→G2 演进序列构成第二组长程关联对照，覆盖赛题对关联任务验证的要求。
...
- **四类角色，长程稳定：** Planner / Retriever / Executor / Summarizer 覆盖赛题全部四类角色，稳定执行**不少于 10 轮**连续任务（CoQA 实测 3 段对话合计 38 个问答轮，单段最长 15 轮；来源 runs/coqa_20260724_211318）。
